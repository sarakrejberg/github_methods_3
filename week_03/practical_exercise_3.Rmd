...
pacman::p_load(tidyverse, readbulk, patchwork, lmerTest, rstanarm)
```
...
```
    ...
    _trial.type_: consist of staircase and experiment. Staircase is the control condition while experiment is the experimentel condition. 
    _pas_: Pas stands for the perceptual awareness scale. goes from 1-4. The participant answers how they experienced the stimuli. 1, is no impression of the stimuli. 2, a feeling that something has been shown, 3:almost clear experience, 4: clear experience 
    _trial_:Number of trials for one participant. 
    _target.contrast_, 
    _cue_, 
    _task_,
    _target_type_, 
    ...
    _obj.resp_, 
    _subject_ 
    _correct_. (That means you can ignore the rest of the variables in your description). For each of them, indicate and argue for what `class` they should be classified into, e.g. _factor_, _numeric_ etc.  
...
fourmodel <- glmer(rt.obj ~ 1 +(1 | subject), data = four)

residual <- resid(fourmodel)

four %>% 
  ggplot(aes(rt.obj, residual))+
